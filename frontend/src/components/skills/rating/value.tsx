        ...
        <>
            <div className="container-progress-bar">
                <div className="b-progress-bar position-relative text-center overflow-hidden">
                    {[...Array(arraylength(0, props.value))].map((value, index) => <ProgressActive key={index} />)}
                    {[...Array(arraylength(defaultSkillLvlCount, props.value))].map((value, index) => <ProgressPassive key={index} />)}
                    {props.value !== '' ?
                        <span className="fs-4 d-inline-block align-middle position-absolute fw-bold">{Number(props.value)}</span>
                        :
