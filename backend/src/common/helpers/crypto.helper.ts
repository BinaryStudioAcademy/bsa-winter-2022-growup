...
export const encrypt = (text: string) : string => {
  ...
  const cipher = crypto.createCipheriv(ENCRYPTION_ALG, Buffer.from(ENCRYPTION_KEY), iv);
...
export const decrypt = (text: string) : string => {
  ...
  const decipher = crypto.createDecipheriv(ENCRYPTION_ALG, Buffer.from(ENCRYPTION_KEY), iv);
